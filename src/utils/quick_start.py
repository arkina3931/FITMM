...
def _resolve_model_name(config):
    model_name = config['model']
    ablation_variant = config['ablation_variant']
    if model_name == 'FITMM' and ablation_variant not in (None, '', 'full'):
        return 'FITMM_ABLATION'
    return model_name
...
def quick_start(model, dataset, config_dict, save_model=True, mg=False):
    ...
    for hyper_tuple in combinators:
        ...
        if resolved_model_name != config['model']:
            logger.info(
                'Auto-switch model loader: %s + ablation_variant=%s -> %s',
                config['model'],
                config['ablation_variant'],
                resolved_model_name,
            )
        model = get_model(resolved_model_name)(config, train_data).to(config['device'])
        logger.info(model)
